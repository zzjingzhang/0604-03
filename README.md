...
项目已包含完整的迁移文件（users、properties、contracts、user_messages 等业务应用），
执行 migrate 后会自动创建：用户表、房源表、收藏表、租赁申请表、合同表、消息表等所有业务表。
...
### 用户审核机制
- 租客和房东注册后默认进入**待审核**状态，无法登录
- 管理员登录后在"用户审核"页面查看待审核用户，可通过或拒绝
- 只有审核通过的用户才能正常登录并使用对应角色的功能
- 管理员账号只能通过 `createsuperuser` 创建，不允许通过注册接口创建

### 权限控制说明
- **房源管理**：只有房东可以发布和管理自己的房源
- **收藏/申请**：只有租客可以收藏房源和提交租赁申请
- **申请审核**：房东只能审核自己房源收到的租赁申请
- **平台审核**：只有管理员可以审核房源、审核用户、查看所有申请
- 所有权限在后端进行校验，前端路由守卫仅作为辅助

### 注意事项
1. 首次运行需要执行数据库迁移
2. 管理员账号需要通过 `createsuperuser` 创建，注册接口无法创建管理员
3. 新注册用户需要管理员审核通过后才能登录
4. 前端通过代理访问后端 API，避免跨域问题
