...
import { MessageSquare, Mail, Send, Check, XCircle } from 'lucide-react';
...
const TenantMessages = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setUsers(response.data.filter(u => u.role === 'landlord'));
    } catch (error) {
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {messages.map((msg) => (
            <div
              key={msg.id}
              className={`bg-white rounded-lg shadow p-6 ${!msg.is_read ? 'border-l-4 border-blue-500' : ''}`}
            ...
            <form onSubmit={handleSendMessage}>
              <div className="mb-4">
                ...
                >
                  ...
                  {users.map(u => (
                    <option key={u.id} value={u.id}>{u.username}（房东）</option>
                  ))}
                </select>
              </div>
              <div className="mb-4">
